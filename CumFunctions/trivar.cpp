...
double trivarcumnorm(double x, double y, double z, double correl12, double correl13, double correl23)
{
	...
	{
		TVCN = bivar::bivarcumnorm(limit[1], utilities::min(limit[2], limit[3]), correlation[2][1]);
	}
	else if (correlation[3][2] + 1 < epsilon)
	{
		if (limit[2] > -limit[3])
		{
			TVCN = bivar::bivarcumnorm(limit[1], limit[2], correlation[2][1]) - bivar::bivarcumnorm(limit[1], -limit[3], correlation[2][1]);
		}
		else
		{
			TVCN = 0;
